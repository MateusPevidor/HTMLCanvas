...
class HamiltonianPath {
  ...
  removeCycle(n, avoid, isHead) {
    const neighbors = this.graph.getNeighbors(n, avoid);

    this.graph.removeNeighbor(n, neighbors[0]);
    if (isHead) {
      this.head = neighbors[0];
    } else {
      this.tail = neighbors[0];
    }
    const { hasCycle, visited } = this.graph.DFS(this.head);
    if (!hasCycle && visited.length == this.size ** 2 - this.obstacleCount) return;
    
    this.graph.addNeighbor(n, neighbors[0]);
    this.graph.removeNeighbor(n, neighbors[1]);
    if (isHead) {
      this.head = neighbors[1];
    } else {
      this.tail = neighbors[1];
    }
